    ...
    public class MasterChicCutServiceController : BaseController
    {
        ...
        public ActionResult _SearchService(Master_ChicCut_ServiceViewModel model)
        {
            var list = (from p in _context.Master_ChicCut_ServiceModel
                        ...
                        where (model.ServiceName == null || p.ServiceName.Contains(model.ServiceName))
                               ...
                               && h.Actived == true && sc.Actived == true
        ...
        public ActionResult APriceList()
        {
            ...
            var lst = (_context.Master_ChicCut_ServiceCategoryModel.Where(p => p.Actived == true).Select(p => new Master_ChicCut_ServiceCategoryViewModel()
            {
                ...
                ServiceName = p.ServiceName,
                OrderBy = p.OrderBy,
                Services = (from s in _context.Master_ChicCut_ServiceModel
                            join h in _context.Master_ChicCut_HairTypeModel on s.HairTypeId equals h.HairTypeId
                            where h.Actived == true && s.ServiceCategoryId == p.ServiceCategoryId
                            select s
                            ).ToList()
            }))
               .OrderBy(p => p.OrderBy).ToList();
            ...
            //Load loại giá lên bảng giá dịch vụ
            ViewBag.HairTypeList = _context.Master_ChicCut_HairTypeModel.OrderBy(p => p.OrderIndex).Where(p => p.Actived == true)
                                    .Select(p => new Master_ChicCut_HairTypeSeachModel()
                                    {
