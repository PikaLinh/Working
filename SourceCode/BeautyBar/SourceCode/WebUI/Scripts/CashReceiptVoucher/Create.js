...
function CalDebt(NumberDebtOld) {
    ...
    var TransactionTypeCode = $("select[name='TransactionTypeCode']").val(); 
    var ContactItemTypeCode = $("select[name='ContactItemTypeCode']").val();
    var dau;
    if (TransactionTypeCode == "TCCHI") {
        dau =  ContactItemTypeCode == "NCC" ? -1 : 1;
    }
    else {
        dau =  ContactItemTypeCode == "NCC" ? 1 : -1;
    }
    var Amount = $("#Amount").val() == '' ? 0 : $("#Amount").val();
    if (Number(NumberDebtOld) + (Amount * dau) < 0) {
        $("#Amount").val(0);
        CalDebt(NumberDebtOld);
    }
    else {
...
$(document).on("click", "#btnSave", function () {
    if ($("select[name='CustomerId']").val() == null && $("select[name='SupplierId']").val() == null && $("select[name='EmployeeId']").val() == null && $("#ContactItemTypeCode").val() != "KHAC" || $("#Amount").val() == "") {
        ...
        $.ajax({
            type: "POST",
            url: "/CashReceiptVoucher/Save",
            data: data,
            success: function (data) {
                if (data == "success") {
                    window.location.assign("/CashReceiptVoucher/Index");
                }
                else {
