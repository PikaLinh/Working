...
$("#btnAddNewRow").unbind("click").click(function () {
    ...
    $.ajax({
        type: "POST",
        url: "/PreOrderMaster/_CreatelistInner",
        data: data,
        success: function (data) {
            $("#tblImportDetail tbody").html(data);
            TotalPrice();
...
function RemainingAmount() {
    if ($("select[name='PaymentMethodId']").val() == 2 || $("select[name='PaymentMethodId']").val() == 3) { // Nếu là tiền mặt,chuyển khoản thi load số tiền khách trả
        $("#GuestAmountPaid").val($("#ViewBagTotalPrice").val());
    }
    else { // công nợ,chọn PTTT
        $("#GuestAmountPaid").val(0);
    }
