    ...
    public class GrossRevenueReportController : BaseController
    {
                      ...
                      select new ReportInforDetailViewModel()
                       {
                           ...
                           ProductCommission = o.ProductCommission,
                           HolidayCommission = o.HolidayCommission,
                           Profits = (o.Total ?? 0) - (o.SumCOGSOfOrderDetail ?? 0) - (o.Tip ?? 0) - (o.Commission ?? 0) - (o.ProductCommission ?? 0) - (o.HolidayCommission ?? 0)
                       }).ToList();
            if (lst != null)
            {
                ViewBag.RowSum = new ReportInforDetailViewModel()
                {
                    ...
                    AdditionalPrice = lst.Sum(p => p.AdditionalPrice),
                    Tip = lst.Sum(p => p.Tip),
                    Commission = lst.Sum(p => p.Commission),
                    ProductCommission = lst.Sum(p => p.ProductCommission),
                    HolidayCommission = lst.Sum(p => p.HolidayCommission),
        ...
        public ReportInforViewModel ListView(SqlDataReader s)
        {
            ...
            try
            {
                ...
                if (s["AdditionalPrice"] != null)
                {
                    ret.AdditionalPrice = decimal.Parse(s["AdditionalPrice"].ToString());
                }
                ...
                if (s["ProductCommission"] != null)
                {
                    ret.ProductCommission = decimal.Parse(s["ProductCommission"].ToString());
                }

                if (s["HolidayCommission"] != null)
                {
                    ret.HolidayCommission = decimal.Parse(s["HolidayCommission"].ToString());
                }
