...
$("#btnAddNewRow").unbind("click").click(function () {
    ...
    $.ajax({
        type: "POST",
        url: "/CRMEmailTemplate/_CreatelistInner",
        data: data,
        success: function (data) {
            $("#tblEmailPara tbody").html(data);
        }
    });
});
...
$(document).on("click", ".detail-btndelete", function () {
    ...
    var EmailParameterId = $("input[name='detail[" + (removeId - 1) + "].EmailParameterId']").val();
    ...
    if (( EmailParameterId  != 0 ) && CheckReferenced == "true") {
        ...
        $.ajax({
            type: "POST",
            url: "/CRMEmailTemplate/_DeletelistInner?RemoveId=" + removeId,
            data: data,
            success: function (data) {
                $("#tblEmailPara tbody").html(data);
            }
...
function SubmitForm(action) {
    ...
    if ($("#EmailTemplateName").val() == "" || $("#EmailTitle").val() == "" || CheckEmailparaEmpty == true) {
        ...
        $.ajax({
            type: "POST",
            url: "/CRMEmailTemplate/" + action,
            data: data,
            success: function (data) {
                if (data == "success") {
                    window.location.assign("/CRMEmailTemplate/Index");
                }
                else {
